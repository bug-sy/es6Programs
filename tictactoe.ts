...
for(let t = 0; t<9; t++){
    ...
    if( (token % 2) === 0 )
    {        
        let position = readline.question("Player number 1 position ");
        let breakingCond = ticTacToe.place(position ,'x')
        token = token + 1;
       if( breakingCond === true)
       {
        ...
        let position = readline.question("Player number 2 position ");
        let breakingCond = ticTacToe.place(position ,'o')
        token = token + 1;
        if( breakingCond === true)
        {
            break;
